...
ヒューマノイド機種の写真をここに置きます。詳細ページには **7スロットのカルーセル** があり、写真が無いスロットは役割名のラベル（`[ MAIN IMAGE ]` 等）が表示されます。
...
- 一覧カード：`transparent ?? hero ?? heroImage` の順に使い、PC枠は **7:6**、画像は `object-contain` で全体を収める（余白は同画像の拡大ぼかし背景で埋める）

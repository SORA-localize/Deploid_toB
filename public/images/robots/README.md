...
- カルーセル本体：幅100%、高さ280px、`sm` 360px、`md` 以上420px。元画像は1600×900px程度を推奨
- 一覧カード：表示可能な `transparent` → `hero` → その他roleの順に代表画像を解決し、PC枠は **7:6**、画像は `object-contain` で全体を収める（余白は同画像の拡大ぼかし背景で埋める）
- 同じ代表画像resolverを一覧、比較、OGP、JSON-LDで使うため、各roleは中央被写体で余白を持たせると安全

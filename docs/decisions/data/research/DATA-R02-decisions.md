...
## 8a. R02-04 スキーマ判断（Option A採用）

Approved: 2026-07-18 / project owner

aeo pilot（R02-03）で「公式情報はあるが現行schemaへ表示できなかった事実」を棚卸しした結果:

- `currentFeatures`（aeoで7件）: 既存`description`は自由記述であり、必要なら個別に一文を追記できる。専用の構造化フィールドを新設しなくても表示は可能。
- 片腕DoF等、全身スカラーへ意味を保って投影できない数値: 表示を諦める（誤読を招く値を書かない）。無理に新フィールドを作らない。
- `usageExamples`（最大3件）: 既存`usageExampleSourceUrls`で表現可能、追加フィールド不要。
- `procurement`（RaaS/国内窓口/保守）: 既存`procurementModels` / `japanAvailability` / `distributorJapan` / `supportNote`で表現可能、追加フィールド不要。

判断: **Option A（現行モデルを維持し、表示対象を既存フィールドへ限定する）を採用する。** Option B（`featureFacts`等の新フィールド追加）は見送る。

理由: 新フィールドを追加すると、61機の一括反映前にvalidator・型・UI・表示上限・重複禁止ルールを別途設計する必要があり、実装が遅延する。既存フィールドで大半（activityStatus、用途、価格、荷重、調達、活用事例URL）は表現可能であり、投影できない情報（currentFeaturesの一覧、per-armのみのDoF等）は無理に構造化せず捨てる。将来必要になれば別途DEC-04として再提案する。

適用範囲:

- `data/types.ts` / `lib/specSchema.ts` / UIコンポーネントは変更しない
- R02の`currentFeatures[]`・`identity.lifecycleStatus`・構造化`procurement`は、そのままの形では`data/*.ts`へ投影しない（棄却ではなく「今回は保存先がない」の意味。docs/data/DATA-R02-*には調査記録として残る）
- 既存フィールドへ意味を保って投影できる値（specs、priceOffers、loadRatings、procurementModels、japanAvailability、distributorJapan、supportNote、usageExampleSourceUrls、UseCase.candidateRobots）は、通常どおりmanifestの`set`/`merge-use-case-relation`/`replace-usage-example-urls`/`add-source`に従って反映する

次段階: R02-05〜06（スキーマ・UI拡張）は実施しない。R02-08（低リスクバッチ反映）へ直接進む。

...
| `sources[].publishedAt` | **表示されない**。取得はするが出典欄のUIには出さない社内記録用の項目 | `SourceList.tsx`のJSXに`publishedAt`参照が無い |
...
| `heroImage`（`src`/`alt`/`credit`/`sourceUrl`/`aspectRatio`） | **`Articles`のみ表示される**（記事カード・トップのニュースカルーセル・JSON-LD画像）。`Manufacturers`/`Robots`/`RobotSeries`/`UseCases`/`Deployments`では**現状どのページからも読まれていない**——各collectionは別の専用画像field（`Manufacturers.logos`、`Robots`/`RobotSeries`の`images`）を使っている | `components/NewsCard.tsx`・`NewsHeroCarousel.tsx`・`NewsFeatureCard.tsx`・`lib/jsonLd.ts`。他collectionの詳細ページにheroImage参照なし。`lib/content/cacheDependencies.ts`の`KNOWN_GAPS`コメントも同じ整理を既に明記している |
...
| field | 公開ページでの扱い |
|---|---|
| `candidateRobots` | **表示される**。用途詳細ページの「候補ロボット」欄 | `use-cases/[slug]/page.tsx` |
...
| field | 公開ページでの扱い |
|---|---|
| `dataAsOf` | **表示される**。`/for-manufacturers`ページの「データ基準時点」表記 |

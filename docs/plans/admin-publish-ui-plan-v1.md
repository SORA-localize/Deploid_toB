...
### D-0. tokenが要る前提（2026-09-03 確認）

**本番の管理者は現在1人**（`platform-admin` 1件、read-onlyで確認）。
token機構が防ぐ「Aが保存 → Bが保存 → Aの公開でBの内容が出る」競合は、
厳密には**編集者が2人以上**、または**同一人物が複数タブで編集**しているときに起きる。

着手時にこの人数を確認しなかったのは手順の不備だった。防御を設計する前に
「その脅威が実際に起きうるか」を確かめるべきだった。

**それでもtokenを維持する。** 理由は2つ。

1. **今後複数人での管理が想定される**（2026-09-03 ユーザー確認）。編集者が増えた時点で
   この競合は現実の問題になる。後から足すには同じ16列のmigrationを本番へ打つ必要があり、
   そのときには公開運用が始まっているぶん手順が重くなる
2. **1人運用でも複数タブでは起きる。** タブAで保存 → タブBで保存 → タブAで公開、で
   意図しない内容が公開される。`assertApprovedVersionIsStillLatest` は
   「最新versionを公開する」ことしか保証しないのでこれを防げない

列は常に `null` で、mapper が明示列挙方式のため公開系・snapshot・parityには現れない
（Global Constraints）。維持コストは小さい。

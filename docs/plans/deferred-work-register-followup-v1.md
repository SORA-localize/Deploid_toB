...
**2026-08-06更新: CSVがHTML形式で再取得され、URL問題が解決した。** 元のCSVはハイパーリンクを保持しない形式だったため出典URLが失われていたが、ユーザーがGoogle SheetsからHTML書き出し（`~/Downloads/ロボDB/ロボDB/発表済みロボット.html`ほか計5ファイル）を提供した。HTMLの`<a href>`としてリンクが残っており、Python（BeautifulSoup）で実際に抽出できることを確認した（198データ行、ロボット名セルに公式製品ページURLが個別に付与されている）。

現行`data/robots.ts`の63機と、氏名（`name`）・メーカー双方が完全一致する行のみで突合した結果（部分一致は誤爆する。例: `fourier-gr1`のname `'GR-1'`が部分一致で`unitree-r1`の`'R1'`行に誤マッチしかけたため、この方式は採用せず完全一致のみ採用した）:

- **40/63機が厳密一致**（name完全一致 かつ メーカー名一致）。うち**20機**は現在`batteryCapacityMah`が空で、CSVには値がある: `unitree-g1-edu`(9000)・`unitree-h1`(15000)・`unitree-h1-2`(15000)・`unitree-h2`(15000)・`unitree-h2-edu`(15000)・`unitree-h2-plus`(15000)・`unitree-r1`(6000)・`unitree-r1-standard`(6000)・`leju-kuavo`(6000)・`leju-kuavo5`(6000)・`agibot-a2-ultra`(14400)・`agibot-a2-lite`(14400)・`pal-kangaroo`(15000)・`limx-oli`(9500)・`limx-luna`(10000)・`ubtech-walker-x`(10000)・`ubtech-walker-c`(15000)・`engineai-se01`(1000・公式表記)・`robotera-l7`(15000)・`galbot-g1`(30000)。残り20機はCSVも`未公表`で一致（新規データなし）
- **23機は厳密一致せず、自動採用しない**。大半はCSV側がより細かいvariant名を持つため（例: Deploidの`booster-t1`(name:'T1') に対しCSVは`T1 Basic`/`T1 Standard`/`T1 Customized`の3行、`engineai-pm01`(name:'PM01')に対し`PM01 Commercial`/`PM01 Education`の2行）。**どのvariantがDeploidのレコードに対応するかは製品ページを個別に確認して人間が判断する**（自動で先頭行を採用しない）
- `unitree-g1`は一時的にAgiBotの空欄行（メーカー列が結合セルで空になっている行）と誤って一致候補に上がったが、手動確認で除外した。**メーカー列の空欄はCSV内の結合セル表示（同じメーカーの2行目以降で列が省略される）が原因**で、今後の突合でも同じ罠がある

**Change**: 上記20機について、CSVのURL（`href`）へ実際にアクセスして404/403でないことを確認し（G2/G6）、`sources[]`へ追加した上で値を採用する。23機の要disambiguation分は、CSVの該当variant行を人間が選定してから同様に扱う。

**Completion**: 対象robotId・値・source URL・確認日の一覧ができる。アクセス不能なURLは`hold-source-inaccessible`として別扱いにする。
...
- Task Bの§2.2発見6で挙げたR02側のmAh/Ah生値の機体対応は、grepの行順から推測した部分が残っており、robotId確定値としては扱っていない。一方、ユーザー提供HTML（Task B-1更新分）は名前・メーカー完全一致による機械的突合で20機を確定できたが、**23機は依然未解決**（CSV側のvariant名がDeploidの代表レコードとどう対応するか人間判断が要る）。
- ユーザー提供ファイル（`~/Downloads/ロボDB/`）はリポジトリ外にあり、この計画はその時点のスナップショットで判定している。ファイルが今後更新される可能性、および完全一致に頼った突合のため表記ゆれ（全角/半角・スペース・variant名の粒度差）で本来一致すべき行を取りこぼしている可能性がある（23機の「不一致」の中に、本当は同一機体だが表記が違うだけのものが混じっている）。
- 同時に提供された`代理店とか.html`・`導入事例＿世界地図用.html`・`未発表ロボット.html`・`モデル突合マスタ（内部）.html`は存在とハイパーリンク保持を確認したのみで、内容は分析していない（このプランの対象4項目に直接関係しないため）。`モデル突合マスタ`はDeploidのrobotIdとの対応表ではなく、CSV内部のセル参照（Sheetsの別タブへのリンク）だった。

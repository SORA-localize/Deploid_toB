...
この文書は `docs/README.md` の「(c) 未実装・作業計画」に属する。
...
3. `docs/decisions/copyright_and_media_rights_policy_v1.md`
4. `docs/decisions/data/README.md`, `docs/decisions/data-maintenance-checklist-v1.md`
5. 本計画
...
公式ページ、公式media/news、利用規約、原典画像URLを確認する読み取り専用Web調査は、
台帳・同期・audit・許諾SSOTの実装前でも実施できる。
ただし、その成果物は候補発見用の非正本であり、権利許諾、公開可否、最終roleを確定しない。

§12.2のgateが禁止するのは、候補binaryの保存・取得、`public/`や公開dataへの登録、
既存rightsの変更、実際の許諾依頼送信である。読み取り専用Web調査の停止条件ではない。

Robot B1〜B6の読み取り専用調査は2026-07-15に完了し、
`robot-image-sourcing-research-B1.md`〜`B6.md`へ保存した。
61機体を重複・欠落なく覆うが、調達台帳・許諾SSOTの代替にはしない。

実行順は次に固定する。

```text
Robot B1〜B6の読み取り専用候補調査（完了）
  -> 調査成果物を非正本として保存
  ->
現行カタログ改修PRを完了
  -> 最新mainから素材調達foundation branch/worktreeを作成
  -> MEDIA-001, 003, 002, 004を実装
  -> §12.2のgateを全件確認
  -> B1〜B6調査成果物をledger candidateへimport・再検証
  -> Robot B1の1メーカーだけ素材取得pilot
  -> 人が台帳、権利判断、画像同一性、取得結果をreview
  -> 残りのRobot/Logo素材取得batchを開始
```

foundation実装前の読み取り専用調査では、画像を保存せず、既存dataやrightsを変更せず、
Robot ID、候補ID、原典page URL、asset URL、候補role、機種一致状態、確認日を構造化して残す。
自由形式のURL一覧だけを作らず、後でtargetへ対応付けられる粒度を維持する。
...
公開画像数だけではbootstrapできない。raw dataには過去判断を残す空srcもある。

| raw media宣言 | 全件 | srcあり | src空 | blockedかつsrc空 |
|---|---:|---:|---:|---:|
| `Robot.images.*`（draft/archived含む） | 72 | 27 | 45 | 12 |
| Manufacturerのlegacy logo / heroImage / logos.* | 56 | 8 | 48 | 4 |

- Robotのsrcあり27件はpublished 26件とdraft `pudu-d7` 1件。
- Robotのsrc空45件には、単なる未取得placeholderと過去の禁止判断が混在する。
- Manufacturerのsrc空48件には、今回のlogo targetへ直接対応しないlegacy logoとmanufacturer heroImageが含まれる。
- 空srcを一律 `unsearched` にするとblocked履歴を失い、一律candidateにすると未取得placeholderを候補と誤認する。

したがってMEDIA-002のbootstrapは、§5.6の規則で個別に分類する。
...
| 素材候補・却下履歴・進捗 | `docs/decisions/data/media/robot-media-collection-ledger.json`（新規） | 候補URL、状態、判断理由 |
...
```ts
interface MediaCollectionLedger {
  schemaVersion: number;
  generatedAt: ISODate;
  sourceCommit: string;
  ownerPolicies: MediaOwnerPolicy[];
  targets: MediaCollectionTarget[];
}

interface MediaOwnerPolicy {
  id: Id;
  ownerKind: 'robot' | 'manufacturer';
  ownerId: Id;
  scope: 'robot-images' | 'manufacturer-logos' | 'manufacturer-hero' | 'all-media';
  lane: 'R1' | 'R2' | 'R3';
  decision:
    | 'unreviewed'
...
`MediaOwnerPolicy` はメーカー・Robot全体へ適用されるterms判断と、target外になったlegacy履歴を保存する。
画像単位の採否や許諾を代替しない。`decision: 'permitted'` だけで個別画像を公開せず、
必ずtarget、candidate、MediaPermissionまで追跡する。
...
- Robot画像targetの `ownerPolicyIds` は、そのRobot自身のpolicy、またはRobotの `manufacturerId` と同じ
  Manufacturerの `robot-images` / `all-media` policyだけを指す。
- Manufacturerロゴtargetの `ownerPolicyIds` は、同じManufacturerの `manufacturer-logos` / `all-media` policyだけを指す。
- `searchHints` は探索開始ページであり、candidateやrights根拠として数えない。
...
### 5.6 既存データのbootstrap規則

同期scriptは既存 `ImageAsset` を次の順で台帳へ移す。空srcをまとめて同じ状態にしない。

| 既存状態 | target / policyへの移行 | binaryの扱い |
|---|---|---|
| srcあり + `own` / `licensed` / `commercial-permitted` | candidateを作り、許諾根拠・機種・variantを再確認後に `published` | 検証通過時だけpublic維持 |
| srcあり + `reference-attributed` / `permission-requested` | candidateを作り `rights-review` | production policyで非表示ならpublic外へ退避 |
| srcあり + `prototype-only` / `blocked` | candidateをrejected、targetまたはowner policyを `prohibited` | publicから除去 |
| src空 + `blocked` | sourceUrl・permissionNoteを履歴candidateまたはowner policyへ移し `prohibited` | binaryなし |
| src空 + `reference-attributed` | sourceUrlを `searchHints` に移し、targetは `unsearched` のまま | binaryなし |
| legacy `Manufacturer.logo` のsrc空 | logo targetのcandidateにしない。sourceUrlだけsearchHints候補 | binaryなし |
| 対象外Manufacturer.heroImageのsrc空 | targetを作らない。blocked/terms履歴だけowner policyへ保存 | binaryなし |
| archived/draft record | active targetを作らない。public binaryとrightsは漏洩audit対象 | policyに従い退避または維持 |

追加規則:

- `permissionNote` の禁止理由を短縮・言い換えで失わない。原文をowner policy/candidate noteへ移す。
- 同じtermsが複数roleに複製されている場合、owner policyを1件作って各targetから参照する。
- empty reference placeholderは「候補発見済み」ではない。
- bootstrap後に既存dataを削除するtaskは別commitにし、台帳へ移ったことを機械比較してから行う。
- `sourceCommit` を保存し、どのdata snapshotから生成した台帳か追跡する。
...
一方、`../decisions/design_system_v1.md` は商用公開に `commercial-strict` を要求している。
...
`../decisions/copyright_and_media_rights_policy_v1.md`, `../decisions/design_system_v1.md`, `lib/media.ts` を同じtaskで整合させる。
AIが独断で方針を選ばない。
...
B1の候補binary取得、staging、公開登録、許諾依頼を始める前に、
実行担当とは別のreviewerが次を確認する。
1項目でも未達なら素材取得batchを開始せず、foundation taskへ戻す。

#### Repository・方針

- [ ] 現行ロボットカタログ改修PRが `main` へmerge済みである。
- [ ] 最新 `main` から素材調達専用branch/worktreeを作り、開始時commitを記録している。
- [ ] production rights policyを人間が決定し、code、`.env.example`、権利方針文書が一致している。
- [ ] direct permission原本の非公開保管責任者と `evidenceRef` の採番規則を決めている。
- [ ] 実際の許諾依頼送信は、ユーザーの明示承認なしに行わないことを担当AIへ伝えている。

#### Foundation成果物

- [ ] `data/mediaPermissions.ts` が存在し、permission参照をvalidatorが検証する。
- [ ] `docs/decisions/data/media/robot-media-collection-ledger.json` が存在し、自由形式メモではなくschema検証される。
- [ ] `scripts/sync-media-targets.mjs` と `scripts/audit-media.mjs` が存在し、npm scriptから実行できる。
- [ ] `.media-staging/candidates/` と `.media-staging/approved/` がgitignoreされ、`public/` 外にある。
- [ ] 非公開status、未参照binary、規格外path、機密証跡をauditが検出する。

#### Target集合・bootstrap照合

- [ ] 実行時点のpublished件数からtargetを再生成し、snapshotが変わらなければ502件である。
- [ ] snapshotではRobot target 427、Manufacturer logo target 75、target key重複0である。
- [ ] B1〜B6がpublished Robot 61件を過不足・重複なく覆う。
- [ ] L1〜L2がpublished Manufacturer 25件を過不足・重複なく覆う。
- [ ] raw `Robot.images.*` 72宣言が、srcあり27件 + src空45件としてbootstrap reportで照合される。
- [ ] raw Manufacturer media 56宣言が、srcあり8件 + src空48件としてbootstrap reportで照合される。
- [ ] src空blockedのRobot 12件、Manufacturer 4件の禁止理由がcandidate historyまたはowner policyへ残る。
- [ ] archived/draftはactive targetに含めず、binaryとrightsをaudit対象に含める。
- [ ] `sourceCommit` が開始時commitと一致する。

#### 安全性・再現性

- [ ] 採用policyで非公開となる既存binaryが `public/` に残っていない。
- [ ] `npm run sync:media-targets` を2回実行して2回目の差分が0である。
- [ ] Web検索を行わないbootstrap dry-runで、既存candidate、blocked理由、permission noteが消えない。
- [ ] `npm run validate:data`, `npm run audit:media`, `npm run build`, `git diff --check` が通る。
- [ ] foundation PRをmergeし、その最新 `main` からB1素材取得pilot専用branch/worktreeを作る。

gate通過記録はPR本文へ、確認日、source commit、件数、採用policy、検証command結果とともに残す。
単に「問題なし」と書いて通過扱いにしない。

### 12.3 計画全体で変更するファイル / 変更しないファイル
...
- `docs/decisions/data/media/robot-media-collection-ledger.json`（新規）
...
- key、slot、状態遷移、不変条件、owner policy参照、重複、件数を検証する。
...
- manufacturer/Robot全体へ適用されるterms判断を `ownerPolicies` に重複なく移し、targetから参照する。
- bootstrap時にraw宣言の総数、src有無、status別移行先をreportし、入力件数と出力件数を照合する。
...
- snapshotのRobot media 72宣言 = srcあり27 + src空45を移行reportで説明できる。
- snapshotのManufacturer media 56宣言 = srcあり8 + src空48を移行reportで説明できる。
- src空blockedのRobot 12件、Manufacturer 4件の判断履歴が移行後も追跡できる。
...
- `docs/decisions/data/media/robot-media-collection-ledger.json`
- `data/robots.ts`
...
- 継続ルールを正本へ反映する。
- 本計画を `docs/archive/` へ移し、README分類を更新する。

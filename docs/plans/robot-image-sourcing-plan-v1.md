...
Last created: 2026-07-08 / Last updated: 2026-07-15（現行データbootstrap・調査開始gateまで改訂）
...
この文書だけを別AIへ渡して、直ちにB1〜B6/L1〜L2のWeb調査を始めてはならない。
現時点では§12.2の調査開始gateを支える台帳・同期・audit・許諾SSOTが未実装だからである。
...
現行カタログ改修PRを完了
  ...
  -> §12.2のgateを全件確認
  -> Robot B1の1メーカーだけpilot調査
  -> 人が台帳、権利判断、画像同一性、報告形式をreview
  -> 残りのRobot/Logo batchを開始
```

foundation実装前にできるのは、現行データ実数の確認、本計画のreview、production rights policyの人間決定までである。
検索結果や候補URLを自由形式メモへ先行収集すると、後でtargetへ対応付けられず再調査になるため行わない。
...
B1のWeb調査を始める前に、実行担当とは別のreviewerが次を確認する。
1項目でも未達ならbatch調査を開始せず、foundation taskへ戻す。
...
- [ ] `docs/data/media/robot-media-collection-ledger.json` が存在し、自由形式メモではなくschema検証される。
...
- [ ] foundation PRをmergeし、その最新 `main` からB1 pilot専用branch/worktreeを作る。

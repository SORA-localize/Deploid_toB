...
| `lib/useCaseEvidence.ts` | 候補ロボット根拠のラベル変換（今回変更なし。primaryDomain 削除の影響を受けないことを確認済み） |
...
| `demo-exhibition` | demonstrate-capability | `marketing` → 削除, `facility` → `facility-management` | `demo`,`exhibition` → `customer-service`（taskTags 空配列はvalidateエラーにならないが、フィルタ表示上タスク不明になるため最低1値を付与する） | `facility-management` |
...
### 2-F. 追加ユースケース一覧（新規追加36シーン・合計44件）
...
| slug | タスク名 | taskTags | maturityLevel | 主な候補robot slug |
|---|---|---|---|---|
| ~~`logistics-tote-sortation`~~ | ~~トート仕分け・パレタイズ~~ | — | — | ※追加しない（既存 `warehouse-tote-material-handling` と重複） |
...
| slug | タスク名 | taskTags | maturityLevel | 主な候補robot slug |
|---|---|---|---|---|
| `construction-cone-placement` | 工事コーン配置 | hazardous-work, material-handling | early-stage | （候補なし）※1 |
...
| `agriculture-fruit-transport` | 果実コンテナ運搬 | material-handling | early-stage | （候補なし）※1 |
| `agriculture-machine-maintenance` | 農業機械メンテ | assembly | early-stage | （候補なし）※1 |
...
| `healthcare-surgery-assist` | 手術補助 | physical-assistance | early-stage | （候補なし）※1 |
...
| slug | タスク名 | taskTags | maturityLevel | 主な候補robot slug |
|---|---|---|---|---|
| ~~`retail-beverage-restocking`~~ | ~~飲料品棚出し~~ | — | — | ※追加しない（既存 `retail-shelf-stocking` と重複） |
| ~~`retail-hotel-reception`~~ | ~~ホテルフロント受付~~ | — | — | ※追加しない（既存 `customer-reception` と重複） |
...
| slug | タスク名 | taskTags | maturityLevel | 主な候補robot slug |
|---|---|---|---|---|
| `facility-floor-cleaning` | 床清掃 | cleaning | early-stage | （候補なし）※1 |
...
| ~~`research-ros-bench`~~ | ~~ROSベンチ・開発環境~~ | — | — | ※追加しない（既存 `research-development` と重複） |
...
**※1 「候補なし」シーンの candidateRobots 方針**

`lib/validate.ts` L564 で `publishStatus === 'published' && candidateRobots.length === 0` はエラー、L569 で公開根拠（`deployment` / `official-use-case` / `adjacent-deployment`）がゼロの場合もエラーになる。
※1 が付いた5シーンは `publishStatus: 'draft'` で追加し、データ整備後に `published` に昇格させること。`draft` 状態では両チェックがスキップされる。
...
- 完了条件:
  ...
  - ステップ2: `npm run validate` が通過し `grep -r "'plant'\|'facility'\|'e-commerce'\|'marketing'\|'healthcare-care'\|'construction-infrastructure'\|'hospitality'\|'public-sector'\|'education'" data/` が 0 件
...
- 完了条件:
  - ステップ1: `npx tsc --noEmit` が通過する
  - ステップ2: `npm run validate` が通過し `grep -r "'r-and-d'\|'hri'\|'exhibition'\|'shelf-stocking'" data/` が 0 件
...
- 完了条件: `grep "'healthcare-care'\|'construction-infrastructure'\|'public-sector'\|'plant'\|'hospitality'\|'education'\|'facility'\|'marketing'" data/robots.ts` が 0 件（`facility` は L2578、`marketing` は L1004・L2512 に存在確認済み。`e-commerce` は robots.ts に存在しないことを確認済み）
...
- 完了条件: `grep "'r-and-d'\|'hri'\|'exhibition'\|'shelf-stocking'\|'quality-inspection'\|'disaster-response'\|'demo'\|'control'" data/robots.ts` が 0 件（文字列リテラルとしての出現のみを検索し、import文・型定義・コメントの誤検知を防ぐ）
...
- 完了条件: 型エラーなし、`grep "primaryDomain\|use-case-domain\|filters\.domain\|'domain'" src/app/use-cases/page.tsx` が 0 件
...
- 変更内容:
  ...
  - 産業タブの値リストは `lib/tags.ts` の既存 `getUseCaseIndustryTagOptions(useCases)` を流用する（新関数を作らない。既存のリスト順・ラベル解決を維持するため）
  ...
  - 産業タブには `role="tablist"` / 各ボタンに `role="tab"` / `aria-selected` を付与し、Left/Right キーでタブ移動できるようにする
- 完了条件: `npm run build` が通過 / ブラウザで産業タブ切替・タスク絞り込み・成熟度別表示が動作する / `aria-selected` が選択中タブに付与されている / キーボードの Left/Right で産業タブを移動できる
...
- 完了条件: カードに maturityLevel バッジが表示される / 候補ロボット名（最大2件）が「候補 N 件」の代わりに表示される
...
2-F で定義した新規追加36シーンを産業ごとに分けて追加する。取り消し線付き slug は既存と重複するため追加しない（2-F の※注参照）。追加は36シーン。

各 commit の前に `npm run validate` を実行し、通過後に commit すること。

各エントリに必要な最低限フィールド:
- `id`, `slug`, `title`, `titleJa`, `subtitle`, `publishStatus: 'published'`（※1 シーンは `'draft'`。2-F ※1 注記参照）
...
| タスク ID | 対象産業 | 新規追加シーン数 | 依存 | 完了条件 |
|---|---|---|---|---|
| E-01 | manufacturing | 5 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / `grep "factory-sheet-metal-supply\|factory-assembly-kit-transport\|factory-powder-weighing\|factory-visual-inspection\|factory-machine-tending" data/useCases.ts \| wc -l` が 5 |
| E-02 | logistics | 4 | A-01, A-02, B-09, B-09b, B-09c | `npm run validate` 通過 / 4 slug が data/useCases.ts に存在 |
| E-03 | construction | 5 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 5 slug が存在（うち `construction-cone-placement` は publishStatus: 'draft'） |
| E-04 | agriculture | 5 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 5 slug が存在（うち fruit-transport, machine-maintenance は publishStatus: 'draft'） |
| E-05 | healthcare | 5 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 5 slug が存在（うち surgery-assist は publishStatus: 'draft'）。既存 `care-physical-assistance`（移乗介助）は別シーンのため維持・追加しない |
| E-06 | retail | 3 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 3 slug が存在 |
| E-07 | facility-management | 5 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 5 slug が存在（うち floor-cleaning は publishStatus: 'draft'） |
| E-08 | research | 4 | A-01, A-02, B-09, B-09b, B-09c, D-01 | `npm run validate` 通過 / 4 slug が存在 |
...
C-02 ─→ C-03（並行可能。UseCaseCard.tsx のみ変更）

D-01 は A-01 ステップ1 完了後に実行する（並行不可。新規ロボットの industryTags に `healthcare`・`facility-management` 等を使うため、A-01 ステップ1 でこれらが tagRegistry に追加されていないと validate がタグ値不明エラーになる）。E タスクは D-01、B-09、B-09b、B-09c 完了後。
```

同じファイルを触るタスクのペア（逐次実行すること）:
- A-01, A-02, A-03 は `lib/tagRegistry.ts` を触るため逐次
- B-09b と B-09c は `data/robots.ts` を触るため逐次（並行実行するとコンフリクト）
...
- `lib/facetConfig.ts` に `USE_CASE_FACETS` を追加する（今回は `SelectControl` で実装し FacetFilterBar への統一は別計画に委ねる）
...
# 削除済みタグ値の残存確認（全削除対象を網羅）
# 産業タグ削除値
grep -r "'plant'\|'facility'\|'e-commerce'\|'marketing'\|'healthcare-care'\|'construction-infrastructure'\|'hospitality'\|'public-sector'\|'education'" data/useCases.ts data/robots.ts
# タスクタグ削除値
grep -r "'quality-inspection'\|'shelf-stocking'\|'r-and-d'\|'hri'\|'control'\|'demo'\|'exhibition'\|'disaster-response'" data/useCases.ts data/robots.ts
# フィールド削除確認
grep -r "primaryDomain\|secondaryDomains" data/useCases.ts
# ※ クォート込みで文字列リテラルのみ検索して誤検知を防ぐ。フィールド名検索はクォート不要
```
...
- [ ] フィルタなしで全44件程度（新規追加36件 ＋ 既存8件維持。重複シーンは既存 slug を維持し新規追加しない）が表示される

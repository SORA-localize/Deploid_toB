...
| 計画 | 一言 | branch | 開始日 |
|---|---|---|---|
| [Admin公開UI](plans/admin-publish-ui-plan-v1.md) | Payload Admin の Publish ボタンから公開できるようにする。**現状は `publish-approval-required` が 500「Something went wrong.」になり公開経路が無い**。`draft=true` 保存 → token で版を束縛 → `publishApprovedVersion()` | `feat/admin-publish-ui` | 2026-09-03 |

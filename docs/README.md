...
| [Admin運用UX・再検証の是正](plans/admin-ux-and-revalidation-fix-plan-v1.md) | Preview運用で見つかった5症状（反映されない・CMSが落ちる・field名が英語 等）の調査と修正計画。**T0〜T6完了**（pooler mode是正、DB接続失敗の分岐、再検証結果のtoast通知、反映SLO文書化、全fieldへのja/enラベル付け、全selectの選択肢ラベル付け、Manufacturersでの編集画面tabs/sidebar化POC）。残るT7（admin.description書き換え）〜T8（実機総合確認）は未着手 | `feat/admin-publish-ui` | 2026-09-04 |

...
### 6.1.1 汎用テーマ画像（実機ではない記事用イメージ）

ロボット本体やメーカー公式画像が使えない記事（政策、資金調達、労使関係など）では、実機を描写しない汎用イメージで代替できる。

- 用途：特定のロボット・製品を映さない、記事テーマを示すだけの背景的な画像（工場フロア、倉庫、政府の建物など）
- 素材：Unsplash License、Pexels Licenseなど、商用Web利用が明示され、帰属表示が必須でない素材を優先する
- Pexels Licenseも個人・商用利用可（帰属表示不要）。ただし「単体素材としての再販」「商標・サービスマークとしての利用」は禁止（`pexels.com/license/`）。Unsplashと同様の運用でよい
- 配置：`public/images/article-generic/<テーマ>/` にローカル保存する（ホットリンク禁止はロボット写真と同じ）。テーマフォルダは記事個別ではなく複数記事で再利用する前提
- 禁止：特定メーカーのロゴ・商標が偶然写り込んだ画像は使わない（無関係企業のブランドが記事に映ると誤認を招く）。例外は、その企業自身が権利者として商用ライセンスで公開した画像（例: メーカー公式アカウントがUnsplashに上げた写真をその企業の記事に使う場合）
- `rights.status: 'licensed'`、`sourceType: 'third-party'`、`rightsHolder` に撮影者名、`licenseUrl` にライセンスページを記録する
- `heroImage.credit` に「撮影者名 / Unsplash」の形でクレジットを残す（法的必須ではないが慣行として表示する）

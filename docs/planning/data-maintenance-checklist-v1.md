...
12. [ ] 記事タグは軸別に入れる。`section` が主題（必須・タブ）、`themeTags` は `kind:'theme'` から任意0〜4個（論点）、`industryTags` は `kind:'industry'`、`regionTags` は `kind:'region'` を使う。`deployment`/`policy` は section が正本で theme に入れない。UIの空タブや件数都合で `section` / `themeTags` を選ばない。企業名・機種名はタグにせず `relatedManufacturerIds` / `relatedRobotIds` で結ぶ。用途との関連もタグから自動生成せず `relatedUseCaseIds` で結ぶ

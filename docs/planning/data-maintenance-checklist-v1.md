...
11. [ ] published の `sources` は空不可（自動）。用途本文と candidate evidence の根拠になる公式/一次情報・信頼できる報道を載せる
12. [ ] `capabilityNotes` / `environmentRequirements` / `whyHardToday` / `japanDeploymentConditions` は各2-3文程度に圧縮する（detail page側で視覚的にも補足扱いにする）
13. [ ] `featuredRank`（任意・数値）は一覧の「注目の適用領域」での優先順位。値が小さいほど上位、未設定は非掲載。実証事例が厚い用途から優先的に設定する（配列順には依存しない。`lib/useCaseFilters.ts`参照）
14. [ ] build 通過 → `published`

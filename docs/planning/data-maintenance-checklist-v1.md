...
12. [ ] 記事タグは軸別に入れる。`themeTags` は `kind:'theme'` から1〜4個、`industryTags` は `kind:'industry'`、`regionTags` は `kind:'region'` を使う。企業名・機種名はタグにせず `relatedManufacturerIds` / `relatedRobotIds` で結ぶ

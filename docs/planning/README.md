...
Last cleaned: 2026-07-01

このディレクトリは、Deploid の現在有効な設計判断と運用方針を置く場所です。
実装済みの一時計画や過去の移行計画は `docs/archive/` に退避しています。
...
- `robot-image-sourcing-plan-v1.md` — 公開Robot×7 role・公開Manufacturer×3 logo variantをMECEなtargetへ分解し、候補・権利・許諾・公開状態を管理する素材調達実行計画
...
`docs/archive/` は実装済み・履歴参照用の計画書置き場です。

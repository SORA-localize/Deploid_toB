...
>
> **現行実装メモ（2026-06-13）**: 本書は再設計前の方針を含む旧ガイド。現行の正本は `data/types.ts` と `docs/planning/data-architecture-redesign-v1.md`。現在は参照を `id`、公開URLを `slug` で分離する。
...
- `safetyNote`
- `vendorRiskNote`
- `UseCase.candidateRobotIds` から逆引きする用途関係
- `comparison`
...
| `industryIds` | `UseCase.candidateRobotIds` または `industryTags` |
...
- `credit` / `sourceUrl` は許諾の証跡ではない。公開用の画像・ロゴは `copyright_and_media_rights_policy_v1.md` に従い、`rights.status` と表示ゲートで管理する。

...
Pudu Roboticsはこれまで配膳ロボット・清掃ロボットといったサービス業向け機体で実績を積んできた企業で、ヒューマノイド専業メーカーとして立ち上がった企業ではない。この経緯は、D7の設計判断にも表れている。研究開発段階の二足歩行機を一から作るのではなく、既存の自律移動・センシング技術を産業用途向けに転用し、ペイロードと稼働時間を伸ばす方向に振っている。Robot.comのR-noidが配達ロボット事業の運用知見を職場向けロボットに転用した事例と同様、ヒューマノイド市場では「既存ロボット事業の隣接領域への展開」が、ゼロから二足歩行機を開発するより速い立ち上がりの手段になりつつある。導入企業にとっては、メーカーの出自が配膳・清掃・配送のどこにあるかを見ることで、得意とする現場条件（屋内平坦面か、屋外段差ありか）をある程度推測できる。`,
  ...
  },
  {
    ...
    manufacturerGuideContent: {
      companyOverview:
        'Unitreeは中国発のロボットメーカーで、四足歩行ロボットの量産で先に実績を作った企業体である。ヒューマノイドでは低価格帯の機体を軸に、研究機関・教育機関・PoCの初期段階の市場で強い。商用の現場での本番運用実績はまだ検証途上で、日本からは代理店経由でのアクセスが現実的な選択肢になる。',
      history:
        '2016年、大学院時代に四足ロボットを研究していた王興興氏がUnitreeを設立した。以降数年は四足歩行ロボットの専業企業として、量産型モデルを継続的に投入する時期にあたる。\n\nGo1などのモデルが低価格帯で市場に広がったことで、「量産と低価格化を両立できる会社」というイメージを固めた。イベント出演や教育現場への導入が進み、四足ロボットの代表格として認知される段階に入っている。\n\nその後、H1を皮切りに事業の軸足を人型ロボットにも広げ、G1・R1と価格帯の異なるヒューマノイドを揃えていった。四足で確立した量産・供給の型を、そのままヒューマノイドに横展開した時期といえる。\n\n直近では、量産実績を背景に資金調達・上場準備が進み、2026年6月1日には上海証券取引所STARマーケットの上場審査を通過した。IPO目論見書によると2025年通期の売上高は16.99億元（約2.5億ドル）、コア事業粗利益率は60.13%で、調達目標は42億元（約6.2億ドル）、上場後の想定評価額は420億元（約62億ドル）とされる。同時に、米国の安全保障政策の対象としても名前が挙がるようになり、事業の評価軸に地政学リスクが加わり始めている。',
      productLineup:
        '現行ラインナップは、用途で束ねると分かりやすい。\n\nG1は研究・教育・PoCの標準候補となる機体で、13,500ドル程度からと価格を抑えつつ、二次開発向けのEDU構成も用意されている。フルサイズのH1・H2は運動性能や産業用途の検証を重視する機体で、価格帯はG1より上位に位置する。最も安価なのはR1 AIR（4,900ドル程度から）で、上位構成のR1は5,900ドル程度からとなる。教育・実験・展示など「まず触れてみたい」用途に向く価格帯だ。2026年にNVIDIA・Sharpaとの協業で発表されたH2 Plusは、Unitree公式ショップで10万ドルという確認済みの価格が付いており、単体のロボットというよりAI開発プラットフォームとして位置づけられる、価格帯としては最上位の構成である。\n\n詳細な自由度・荷重・稼働時間などのスペックは、各機体のロボット詳細ページで確認するのが適切である。',
      evaluationIntro: '評価は5つの軸に分けて読むのが実務的である。',
      evaluationAxes: {
        pricing: {
          level: 'strength',
          body: 'G1・R1は同クラスのヒューマノイドの数分の1の価格帯で、研究・教育・PoCの初期段階で最も候補に入れやすい水準にある。',
        },
        productionCapacity: {
          level: 'strength',
          body: '四足ロボットで先に量産・供給網を構築した実績があり、ヒューマノイド専業のスタートアップに比べて量産の説明がしやすい。ただし、ヒューマノイド単体の長期商用運用実績とは別の論点である。',
        },
        ecosystem: {
          level: 'strength',
          body: 'NVIDIAが自社のリファレンス基盤の土台としてH2ボディを採用するなど、開発者向けエコシステムとの接続は強い。ただし、これは開発基盤としての評価であり、現場でそのまま使える運用実績を意味しない。',
        },
        trackRecord: {
          level: 'caution',
          body: '第三者企業が本番稼働として検証した事例は、公開情報の範囲では確認できない。話題になった自社工場での組立実証も、Unitreeが自ら公開した動画のみが根拠である。',
        },
        geopoliticalRisk: {
          level: 'risk',
          body: '2026年6月、米国防総省がUnitreeを中国軍民融合に関与する企業として「1260Hリスト」に追加した。日本企業の民生調達を法的に禁止するものではないが、米国の輸出管理・調達方針の変化を注視する材料になる。',
        },
      },
      deploymentIntro:
      ...
      deploymentStatus: {
        ...
        poc: {
          evidence: 'limited',
          body: '顧客名・現場・目的が確認できる事例は、2026年7月時点の公開情報では限定的である。',
        },
        researchEducation: {
          evidence: 'confirmed',
          body: 'NVIDIAとの協業により、スタンフォード大学やチューリッヒ工科大学（ETH Zurich）などの研究機関にH2とJetson Thorが供給されている。',
        },
        exhibitionDemo: {
          evidence: 'confirmed',
          body: 'R1・G1・H1は各種イベント・展示会での実演が多数確認できる。',
        },
        internalTrial: {
          evidence: 'confirmed',
      ...
      deploymentOutro:
        '研究・教育、展示・デモ、社内実証が先行し、商用導入の裏付けはまだないというのがUnitreeの現在地である。',
      japanProcurement:
        '日本国内の窓口はTechShare（Unitree Supported Partner Program参加）、Tohasen Roboticsに加え、2026年6月19日にGMOインターネットグループのGMO AI&ロボティクス商事（GMO AIR）が正規代理店契約を締結し、3社体制になっている。G1についてはTechShareが窓口として案内されており、GMO AIRは販売に加えて導入支援・ソフトウェア開発・保守運用までのワンストップ提供を掲げる。構成はG1/G1 EDU、H2/H2 EDU、R1 AIR/R1/R1 EDUなど細かく分かれ、本体価格に送料・関税・保守費が含まれるかどうかも構成ごとに異なるため、見積の段階で内訳を確認する必要がある。保証期間、修理が国内対応か本国送りか、交換部品の納期、ソフトウェア更新の担当窓口、商用利用時の安全責任の所在、教育用・研究用・商用で契約条件が変わるかどうかは、いずれも公式情報だけでは判断できず、代理店への個別確認が前提になる。加えて、2026年6月に米国防総省がUnitreeを1260Hリストに追加した事実がある。これは米国内の防衛関連調達を対象にした措置で、日本国内の民生調達を直接制限するものではないが、取引先や資金調達に米国企業・投資家が関わる場合は、その影響範囲も合わせて代理店経由で確認しておきたい。',
      fitSummary:
        'Unitreeが候補に入りやすいのは、限られた予算でヒューマノイドを触り始めたい研究機関、教育機関、PoC初期の事業者である。G1やR1は価格帯が低く、他社のフルサイズ機体と比べても検証開始までのハードルを下げやすい。\n\n一方、第三者企業の本番商用ラインで長期稼働した事例を重視する場合、公開情報だけでは判断材料が足りない。地政学リスクやサポート体制の確認が厳格に求められる大企業・上場企業では、代理店経由で保証、保守、部品供給、米国関連規制の影響範囲を確認したうえで比較する必要がある。\n\n比較対象としては、商用導入の進展を見るならAgility RoboticsやFigure AI、低価格帯・研究用途で見るなら他の中国系メーカー、フルサイズ機体の将来性で見るならTesla Optimusなどが候補になる。次に見るべきはG1・H2・R1のロボット詳細ページで、自由度・荷重・稼働時間などの仕様を具体的に比較することだ。問い合わせ前には、構成ごとの価格・納期・保証の内訳と、1260Hリストが自社の取引にどこまで関係するかを整理しておくとよい。',
    },
  },
];

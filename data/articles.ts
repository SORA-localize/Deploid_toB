...
RaaS モデルで最も重要なのは「何を SLA として合意するか」だ。稼働保証（uptime）、故障時の対応時間、世代更新のタイミングと追加費用、日本語対応窓口の有無——これらが契約の価値を決める。現時点でAgilityは国内に正式な販売・保守拠点を持っていないため、本格導入を検討する場合は代理店交渉と合わせて進める必要がある。`,
  ...
  // ── 2026年 最新ニュース ──────────────────────────────────────────
  {
    ...
    title: "JAL launches Japan's first humanoid robot trial at Haneda for ground operations",
    titleJa: 'JAL、羽田空港で国内初のヒューマノイド実証開始——地上業務への適用を検証',
    type: 'deployment-report',
    category: 'news',
    section: 'deployment',
    summary: '日本航空（JAL）が2026年5月、羽田空港で国内航空会社として初めてヒューマノイドロボットの実証実験を開始した。GMO AI & Robotics と連携し、手荷物・貨物の積み降ろし等のグランドハンドリング業務への適用を検証する。',
    publishStatus: 'published',
    ...
    themeTags: [],
    whyItMatters:
      '日本の大手レガシーキャリアが安全規制の厳しい空港現場でヒューマノイドを実運用に踏み切ったことは、国内の他業種への波及効果が大きい。GMO AI & Robotics が現場業務向けの制御・運用を担う構成も、導入モデルとして参照価値が高い。',
    keyTakeaways: [
      '2026年5月開始・2028年まで2年間の実証。3年以内の商用化が目標（JAL公式発表）',
      'GMO AI & Robotics が空港業務向けの制御・運用ソフトを担う構成',
...
今回の実証では、GMO AI & Robotics が空港グランドハンドリング業務向けに動作制御ソフトウェアを開発し、JAL グランドサービスの現場要件に合わせて機体を運用する。公開情報だけでは機体メーカー・モデルを一次情報で確認できないため、Deploid上では特定ロボットとの構造化関連付けは行わない。

「作業特化ソフトを現場側・インテグレーター側で作り込む」というモデルは、国内事業者がヒューマノイドを活用する際の現実的な構成パターンとして参照価値がある。
...
- **機体選択**: 公開情報だけではメーカー・モデルを一次情報で確認できないため、調達条件と保守責任は個別確認が必要

...
Use this file when searching for article candidates (news, press releases, deployments) before writing or adding to the Payload articles collection. This is the "where to look" stage, before `21-data-maintenance-workflow.md` ("how to record it").
...
- `docs/decisions/editorial_style_guide_v1.md` §1.5 — which subjects to prioritize and which to deprioritize; §3 Step 1 — what counts as a usable source once found
...
- Use date-filtered search (recent days/weeks) for hero/news candidates; older or weak-source items go to a research backlog, not directly to Payload.
- For daily automated article generation, use `deploid_daily_article_batch_v1` from `news-automation-prompt-contract-v1.md`; do not paste unstructured Scheduled Tasks output directly into Payload.
- When the user posts a message headed like `日次ニュース` or `今日のニュース` followed by ChatGPT Scheduled Tasks output, treat it as a CLI handoff. The user does not need to paste CLI instructions. Read `news-automation-prompt-contract-v1.md` section 3, perform duplicate checks through the Payload repository before editing, and convert only non-duplicate `articles[].article` items mechanically. Do not perform supplemental news research to fill missing body, sources, or images.
